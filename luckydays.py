...
print('十死（じゅうしび、じゅうしにち）は十死一生日、天殺日などとも言う。受死日に次ぐ大悪日。何事にも悪い日とされ、葬式にも凶。')
...
for a_date in date_ary.values:
    ...
    if nijyuuhassyuku == '鬼':
        events += '、　' + nijyuuhassyuku + '宿日'
    else:
        events += '、　' + nijyuuhassyuku
    ...
    if Jyuushinichi.is_jyuushinichi(a_date):
        events += '、　十死日'
        
    # 天赦日
    if Tenshanichi.is_tensyanichi(a_date):
        events += '、　天赦日'
    
    # 一粒万倍日
    if Ichiryuumanbainichi.is_ichiryuumanbainichi(a_date):
        events += '、　一粒万倍日'
